...
class RailwayParser:
    def __init__(self, use_ai=True):
        self.use_ai = use_ai
        self.equipment_types = config.EQUIPMENT_TYPES
        self.major_sch_types = config.MAJOR_SCH_TYPES
        self.minor_sch_types = config.MINOR_SCH_TYPES
...
{{
    ...
    "data": {{
        ...
        "one_liner_summary": "A highly detailed 1-line summary of the action including all extracted equipment details. Prefix with the action type (e.g., 'FITMENT: Fitted PANTO...', 'REMOVAL: Removed...', 'INFO: ...')"
    }}
}}
...
- Only respond with valid JSON."""
        try:
            response = self.client.chat.completions.create(
                ...
                temperature=0.1,
                max_tokens=1024  # Increased for complex logs
            )
